...
describe("CardPromptStudio", () => {
  ...
  it("does not generate against built-in text while persisted templates are loading", async () => {
    serverFnMock.mockImplementation(() => new Promise(() => undefined));
  ...
  it("keeps clipboard success and warns when initial history persistence fails", async () => {
    serverFnMock.mockRejectedValue(new Error("history unavailable"));
    const user = userEvent.setup();
    ...
    await waitFor(() =>
      expect(toast.warning).toHaveBeenCalledWith("Prompt copied, but history could not be saved"),
    );
  });
  ...
  it("does not associate standalone history with the previously selected player", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Copy prompt" }));
    await waitFor(() => {
      const saveCall = serverFnMock.mock.calls.find(([value]) => value?.data?.kind === "initial");
      expect(saveCall?.[0].data.eventParticipantId).toBeNull();
    });
  });
  ...
  it("claims a batch item before awaiting history persistence", async () => {
    ...
    serverFnMock.mockImplementation((value?: { data?: { kind?: string } }) =>
      value?.data?.kind === "initial"
        ? pendingSave
        : Promise.resolve({ templates: [], runs: [], id: "history-id" }),
    );
    const user = userEvent.setup();
    vi.spyOn(navigator.clipboard, "writeText").mockResolvedValue(undefined);
    renderStudio(
    ...
    await user.click(screen.getByText("Batch Production"));
    ...
    await waitFor(() =>
      expect(
        serverFnMock.mock.calls.filter(([value]) => value?.data?.kind === "initial"),
      ).toHaveLength(1),
    );
