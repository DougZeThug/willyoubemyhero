 ...
 *
 * Every arm is a `{ ok: false, reason }` one of the three set handlers can
 * answer with — they resolve rather than throw, so the refusal arrives down the
 * success arm of the panel's toast and has only the reason code to explain
 * itself from. Two of them used to fall through to the name line: a delete
 * refused because somebody has already finished the set, and an edit to a set
 * another tab had already removed. Both then blamed the name — which a delete
 * never sends — and offered "try letters and numbers", which is no way out of
 * either. Hiding IS the way out of the first, and it is one icon along the
 * same row.
 *
