...
//
// The pull itself lives in pack.functions.ts now. What is checked here is
// everything around it: that the member comes from the token, that a response
// never carries a card the caller has not pulled, and that the response shape
// has no denominator in it — "3 of 12" anywhere gives the set size away just as
// completely as a readable table would.
