-- Restore the named secret cards in the public trade feed.
--
-- 20260825000127 taught accept_trade_offer's summary builder to name the secret
-- that changed hands. 20260825120000 then re-created the whole function from a
-- copy that predated it, and because migrations apply in filename order the
-- older summary won. Both files read correctly on their own, nothing raised, and
-- the feed quietly went back to saying "a secret".
--
-- Re-applying the naming would leave the same trap set, so the summary comes out
-- of the function body entirely. It was two copies of one query — proposer and
-- recipient — which is exactly the shape trade_item_is_spare was extracted for:
-- "Two copies of this rule would drift". Now the next person to re-create
-- accept_trade_offer from a stale copy still calls this, and the naming survives.

-- ============ WHAT THE LEAGUE IS TOLD ============
-- Roster items name the CARD — public data anyone can already browse — resolved
-- from the copy that just moved. Secret items name theirs too: the league asked
-- to read which secret changed hands, and a NAME is the whole of what is widened
-- here. No art path, no flavour, no tier, and nothing at all about a card nobody
-- has traded. The catalogue stays server-only.
--
-- AND NO EDITION, deliberately. Since 20260826120000 a finish is Postgres's own
-- rather than a phone's claim, so the old argument for withholding it is gone —
-- but the new one is simpler and better: this table is anon-readable and
-- realtime-published, and what finish somebody handed over is between the two
-- people in the trade. The league sees a card moved.
--
-- The roster join is effectively INNER via the WHERE: an item whose copy has
-- since been deleted is dropped rather than recorded as a null card. The secret
-- joins are LEFT for the mirror-image reason — a secret whose name cannot be
-- resolved still belongs in the record, and the client falls back to the old
-- "a secret" wording for it.
CREATE OR REPLACE FUNCTION public.trade_summary(
  _offer_id    uuid,
  _giver_side  text
) RETURNS jsonb
LANGUAGE sql
...
AS $$
  SELECT coalesce(jsonb_agg(
    CASE i.kind
      ...
      ELSE jsonb_strip_nulls(jsonb_build_object('kind', 'secret',
                                                'secretCardId', sc.id,
                                                'name', sc.name))
    END ORDER BY i.kind, i.id), '[]'::jsonb)
    FROM public.trade_offer_items i
    LEFT JOIN public.card_copies       cc  ON cc.id  = i.card_copy_id
    LEFT JOIN public.secret_card_pulls sp2 ON sp2.id = i.secret_pull_id
    LEFT JOIN public.secret_cards      sc  ON sc.id  = sp2.secret_card_id
   WHERE i.offer_id = _offer_id
     AND i.giver_side = _giver_side
     AND (i.kind = 'secret' OR cc.id IS NOT NULL);
$$;

REVOKE ALL ON FUNCTION public.trade_summary(uuid, text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.trade_summary(uuid, text) TO service_role;

-- ============ THE ACCEPT ============
-- Body unchanged from 20260825120000 apart from the two summary statements, which
-- now call the helper above.
