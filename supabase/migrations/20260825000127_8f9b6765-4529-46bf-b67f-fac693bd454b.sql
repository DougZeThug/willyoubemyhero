...
BEGIN
  ...
  -- browse — resolved from the copy that just moved. Secret items now name the card
  -- too: the league asked to read which secret changed hands, and a NAME is the whole
  -- of what is widened here. No art path, no flavour, no foil, no tier, and nothing
  -- at all about a card nobody has traded — the catalogue stays server-only.
  --
  ...
  -- The secret joins are LEFT for the mirror-image reason: a secret whose name
  -- cannot be resolved still belongs in the record, and the client falls back to
  -- the old "a secret" wording for it.
  SELECT
    coalesce(jsonb_agg(
      CASE i.kind
...
COMMENT ON COLUMN public.trades.proposer_gave IS
  'PUBLIC-SAFE SUMMARY. Roster items carry their event_participant_id, which is public data on a public card. Secret items carry {"kind":"secret","secretCardId":...,"name":...} — deliberately widened from the old kind-only shape so the league feed can say which secret moved. A NAME only: no art_path, flavour, foil or tier, and only for cards that actually changed hands, so the catalogue that secret_cards being server-only protects is still not enumerable from here. Built only inside accept_trade_offer.';

COMMENT ON COLUMN public.trades.recipient_gave IS
  'PUBLIC-SAFE SUMMARY. Same shape and same rules as proposer_gave.';

-- BACKFILL, so the feed does not read half-named. Rebuilt from trade_offer_items
-- rather than patched in place: the items are the record of what moved, and a
-- trade whose offer rows are gone keeps whatever summary it already has. Roster
-- entries are re-derived identically, so a replay of this file is a no-op on rows
         ...
         coalesce(jsonb_agg(
           CASE i.kind
             ...
             ELSE jsonb_strip_nulls(jsonb_build_object('kind', 'secret',
                                                       'secretCardId', sc.id,
                                                       'name', sc.name))
           END ORDER BY i.kind, i.id), '[]'::jsonb) AS items
    FROM public.trades t
    JOIN public.trade_offer_items i ON i.offer_id = t.offer_id
    LEFT JOIN public.card_copies cc ON cc.id = i.card_copy_id
    LEFT JOIN public.secret_card_pulls sp ON sp.id = i.secret_pull_id
    LEFT JOIN public.secret_cards sc ON sc.id = sp.secret_card_id
   WHERE t.offer_id IS NOT NULL
     AND (i.kind = 'secret' OR cc.id IS NOT NULL)
   GROUP BY t.id, i.giver_side
)
UPDATE public.trades t
   SET proposer_gave  = coalesce(p.items, t.proposer_gave),
       recipient_gave = coalesce(r.items, t.recipient_gave)
  FROM (SELECT trade_id, items FROM rebuilt WHERE giver_side = 'proposer') p
  FULL JOIN (SELECT trade_id, items FROM rebuilt WHERE giver_side = 'recipient') r
    ON r.trade_id = p.trade_id
 WHERE t.id = coalesce(p.trade_id, r.trade_id);
