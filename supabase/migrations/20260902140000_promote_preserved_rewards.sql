-- A preserved reward has to be able to own its card.
--
-- 20260902130000 stopped the guest merges deleting a granted row on a day the
-- destination already spent. It opened a gap in the same statement: when the
-- guest's ordinary pull of X collides and goes, a same-day granted DUPLICATE of
-- X survives it — and every step after that only ever demotes. The destination
-- ends up holding X as `is_duplicate = true` and nothing else, which is a card
-- every count ignores, the trophy sweep skips, and pull_secret_card would deal
-- again as new. resync_secret_ownership exists for exactly this shape on the
-- trade path; both merges now finish with the same promotion, set-based, over
-- the rows that just moved.
