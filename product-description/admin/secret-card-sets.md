...
| The token expires or is cleared | Every control still looks live — a 12-hour session ending is noticed on a timer, not at the moment it happens. The first tap after it produces "Admin PIN required". | An upload batch that expires part-way is the worst case: cards already inserted stay, the rest fail. Re-entering the PIN and re-dropping the remainder is the repair. |
